...
export function ChatBot({ isMinimized, onToggleMinimize }: ChatBotProps) {
  ...
  return (
    <div className="w-96 bg-dark-panel border-l border-dark-border flex flex-col h-screen">
      {/* Header */}
      <div className="p-4 border-b border-dark-border flex items-center justify-between">
        <div>
          <h3 className="heading-inter heading-inter-sm">Chat Assistant</h3>
          <p className="text-gray-400 text-xs mt-1">Ask about alarm procedures</p>
        </div>
        <button
          onClick={onToggleMinimize}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 text-sm mt-8">
            <p>Start a conversation about alarm procedures</p>
          </div>
        )}
